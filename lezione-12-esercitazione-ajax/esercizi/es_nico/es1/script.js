...
//
// Concetti: fetch, await, response.json()
...
btnCarica.onclick = async () => {
  ...
  for( let i=0; i<data.length; i++) {
    ...
    const template = "<tr> <td>%ID</td> <td>%NAME</td> <td>%EMAIL</td> </tr>";
    tbodyUtenti.innerHTML += template.replace("%ID", id).replace("%NAME", nome).replace("%EMAIL", email);
